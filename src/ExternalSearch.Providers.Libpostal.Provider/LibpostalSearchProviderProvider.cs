...
namespace CluedIn.Provider.Libpostal
{
    public class LibpostalSearchProviderProvider : ProviderBase, IExtendedProviderMetadata, IExternalSearchProviderProvider
    {
        public IExternalSearchProvider ExternalSearchProvider { get; }

        public LibpostalSearchProviderProvider([System.Diagnostics.CodeAnalysis.NotNull] ApplicationContext appContext) : base(appContext, GetMetaData())
        {
        ...
        public override async Task<CrawlJobData> GetCrawlJobData(ProviderUpdateContext context, IDictionary<string, object> configuration, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            ...
            var result = new LibpostalExternalSearchJobData(configuration);
        ...
        {
            if (jobData is LibpostalExternalSearchJobData result)
            {
                return await Task.FromResult(result.ToDictionary());
            }

            throw new InvalidOperationException($"Unexpected data type for {nameof(LibpostalExternalSearchJobData)}, {jobData.GetType()}");
        }
